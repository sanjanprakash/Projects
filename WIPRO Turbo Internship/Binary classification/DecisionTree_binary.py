...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.4, random_state = 987, stratify = y)
...
pipeline = make_pipeline(preprocessing.StandardScaler(),DecisionTreeClassifier(random_state = 987))
#print pipeline.get_params().keys()											#Listing out the hyperparameters that can be fine-tuned
hyperparameters = {'decisiontreeclassifier__max_depth' : range(3,1,-1),'decisiontreeclassifier__max_features' : range(len(whole[0]) - 3,25,-1)}	#Choosing the hyperparameters that are to be tuned
...
joblib.dump(clf, 'DecisionTree_binary_classifier.pkl')									#Storing the model, to be used later

#To load the model and test it on another test set
#clf2 = joblib.load('DecisionTree_binary_classifier.pkl')
