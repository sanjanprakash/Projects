...
pipeline = make_pipeline(preprocessing.StandardScaler(),LogisticRegression(max_iter = 10000,class_weight = 'balanced',random_state = 987,solver = 'liblinear'))
#print pipeline.get_params()											#Listing out the hyperparameters that can be fine-tuned
hyperparameters = {'logisticregression__C' : [1e-3,1e-2,1e-1,1e0],'logisticregression__intercept_scaling' : [0.05,0.1,0.5,1.0],'logisticregression__tol' : [1e-8,1e-6,1e-4,1e-2,1e0]}	#Choosing the hyperparameters that are to be tuned
...
joblib.dump(clf, 'Logit_binary_classifier.pkl')									#Storing the model, to be used later

#To load the model and test it on another test set
#clf2 = joblib.load('Logit_binary_classifier.pkl')
