'''
SVMs are useful where the number of features is greater than the number of samples. But if the difference between the 2 is large, the performance could be poor. 
'''
import numpy as np
import pandas as pd
...
pipeline = make_pipeline(preprocessing.StandardScaler(),SVC(max_iter = 10000,random_state = 987))
#print pipeline.get_params()											#Listing out the hyperparameters that can be fine-tuned
hyperparameters = {'svc__kernel' : ['rbf','sigmoid','poly'],'svc__decision_function_shape' : ['ovo','ovr'],'svc__tol' : [1e0,1e-2,1e-4,1e-6,1e-8]}	#Choosing the hyperparameters that are to be tuned
...
joblib.dump(clf, 'SVM_binary_classifier.pkl')									#Storing the model, to be used later

#To load the model and test it on another test set
#clf2 = joblib.load('SVM_binary_classifier.pkl')
