...
pipeline = make_pipeline(preprocessing.StandardScaler(),KNeighborsClassifier(n_neighbors = 2))
#print pipeline.get_params()											#Listing out the hyperparameters that can be fine-tuned
hyperparameters = {'kneighborsclassifier__weights' : ['uniform','distance'],'kneighborsclassifier__algorithm' : ['ball_tree','brute','kd_tree','auto'],'kneighborsclassifier__leaf_size' : range(5,35,5)}
...
joblib.dump(clf, 'kNN_multi_classifier.pkl')									#Storing the model, to be used later

#To load the model and test it on another test set
#clf2 = joblib.load('kNN_multi_classifier.pkl')
