'''
This is an attempt at using Support Vector Machine for Mutinomial Classification.
'''
import numpy as np
import glob
import os
...
pipeline = make_pipeline(preprocessing.StandardScaler(),SVC(max_iter = 100000,random_state = 123,decision_function_shape = 'ovr'))
#print pipeline.get_params()											#Listing out the hyperparameters that can be fine-tuned
hyperparameters = {'svc__C' : [1e0,1e1,1e2,1e3,1e4],'svc__kernel' : ['sigmoid','poly','rbf'],'svc__tol' : [1e1,1e0,1e-2,1e-4,1e-6,1e-8,1e-10,1e-12]}
...
joblib.dump(clf, 'SVM_multi_classifier.pkl')									#Storing the model, to be used later

#To load the model and test it on another test set
#clf2 = joblib.load('SVM_multi_classifier.pkl')
