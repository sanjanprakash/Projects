...
pipeline = make_pipeline(preprocessing.StandardScaler(),SVR(tol = 10))
#print pipeline.get_params()											#Listing out the hyperparameters that can be fine-tuned
hyperparameters = {'svr__epsilon' : [1e-1,1e-3,1e-2,1e-4,1e0,1e-5,1e-6,10],'svr__C' : [1e-3,1e-2,1e-1,1e0,1e1,1e2,1e3],'svr__kernel' : ['rbf','sigmoid','poly','linear'],'svr__degree' : [4,3,2,1]}
...
print "RMSE : ",rmse
joblib.dump(clf, 'SVM_regressor.pkl')									#Storing the model, to be used later

#To load the model and test it on another test set
#clf2 = joblib.load('SVM_regressor.pkl')
