...
pipeline = make_pipeline(preprocessing.StandardScaler(),DecisionTreeRegressor(random_state = 987))
#print pipeline.get_params()											#Listing out the hyperparameters that can be fine-tuned
hyperparameters = {'decisiontreeregressor__max_features' : ['auto','log2'],'decisiontreeregressor__max_depth' : [3,4,5,6,7],'decisiontreeregressor__min_impurity_split' : [1e-9,1e-6,1e-3],'decisiontreeregressor__min_weight_fraction_leaf' : [0.05,0.1,0.15,0.2],'decisiontreeregressor__min_samples_split' : [5,10,15,20]}
...
print "RMSE : ",rmse
joblib.dump(clf, 'DecisionTree_regressor.pkl')			

#To load the model and test it on another test set
#clf2 = joblib.load('DecisionTree_regressor.pkl')
